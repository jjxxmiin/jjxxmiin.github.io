...
2026-09-13 현재 실행 계획은 [성장 실험 운영 지침](GROWTH_PLAYBOOK.md)을 따른다.
아래 8월 수치는 과거 진단 기록이며 최신 기준선은 비공개 `.growth-state/`에 보관한다.

GA4 실측(2026-08-24, 2026-05-27~08-24) 기준 문제는 하나다. 측정 시작 전에
이미 공개돼 90일 전체를 공개 상태로 보낸 514개 중 **447개(87.0%)가 자연 검색 랜딩
세션 0**이었다. 자연 검색은
416세션(하루 4.6회), 참여 세션은 141회(하루 1.6회)였다. 세션당 PV나 광고
배치가 아니라 **유효 검색 유입 자체**가 병목이므로, 여기 도구들은 그 항을 겨냥한다.

2026-08-24부터는 글 수뿐 아니라 인기와 수익 의도를 함께 본다. Hacker News와
DEV의 실제 점수·댓글, GeekNews·요즘IT·Toss Tech·Tom's Guide·Ars·TechCrunch의
인기/편집 신호로 주제를 고르고, `가격·비교·추천·오류 해결`처럼 도입과 결제에 가까운
검색어를 우선한다. 단, 비교·추천 글은 직접 테스트 없이는 자동 발행하지 않는다.
소스 목록, 품질 게이트, 우선 글감은 [EDITORIAL_STRATEGY.md](EDITORIAL_STRATEGY.md)에
정리했다.
...
GA4에는 노출(impressions)이 없다. 447개 URL이 "검색 수요가 없어서" 0인지
