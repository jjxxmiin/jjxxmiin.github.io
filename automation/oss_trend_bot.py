...
# `cd automation && python oss_trend_bot.py` (CI) or `python automation/oss_trend_bot.py`.
POSTS_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "_posts")
FALLBACK_THUMBNAIL = "/assets/img/logo.png"
# Front matter marker: lets this bot (and the workflow) tell its own posts apart from
# the morning AI-news posts, which carry `automation: daily_ai_news`.
AUTOMATION_TAG = "oss_trend"
# Same single production model as the news bot, so both jobs bill at one known rate.
FALLBACK_MODELS = [
    "gemini-3.6-flash",
]
...
def daily_post_exists(*, now=None):
    """Return True once this bot has produced a KST-dated post for today.

    The evening job runs once a day, but workflow retries and manual dispatches would
    otherwise publish a second open-source post on the same day."""
    now = now or kst_now()
    prefix = f"{now:%Y-%m-%d}-"
    ...
    for filename in os.listdir(POSTS_DIR):
        ...
        try:
            ...
            continue
        if re.search(rf"(?m)^automation:\s*{AUTOMATION_TAG}\s*$", head):
            return True
    return False
...
def generate_blog_post(client, topic_data):
    ...
    # Load prompt from config. Kept in its own file: prompt_config.json now belongs to
    # the morning news bot and no longer carries a `daily_trend_bot` prompt.
    config_path = os.path.join(os.path.dirname(__file__), 'oss_prompt_config.json')
    import json
    try:
...
def save_post(post_data):
    ...
    # KST, matching the news bot and daily_post_exists(). The old US-Eastern (UTC-5)
    # stamp dated an evening KST run to the previous day whenever it ran before 14:00 KST.
    now = kst_now()

    date_filename = now.strftime("%Y-%m-%d")
    date_frontmatter = now.strftime("%Y-%m-%d %H:%M:%S")
...
def main():
    force = os.environ.get("OSS_TREND_FORCE_PUBLISH", "").strip().lower() in {
        "1", "true", "yes"
    }
    if not force and daily_post_exists():
        print("오늘의 오픈소스 트렌드 글이 이미 있어 추가 발행하지 않습니다.")
        return
    ...
    if not topics_list:
        raise RuntimeError("GitHub Trending에서 후보를 찾지 못했습니다.")
    ...
    for topic_data in topics_list:
        ...
        if post_data:
            # 4. Save — one post per run
            save_post(post_data)
            return

        print(f"Failed to generate post content for {topic_name}. Trying next...")

    # Swallowing this used to end the run green with nothing published, which gave no
    # red run and no alert mail. Fail loudly so the workflow's retry/alerting kicks in.
    raise RuntimeError("모든 트렌드 후보가 중복이거나 글 생성에 실패했습니다.")
