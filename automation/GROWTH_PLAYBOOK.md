...
- `python -m automation.growth_report`로 같은 길이의 이전/현재 28일 보고서를
  `.growth-state/`에 저장한다. 기본 종료일은 3일 전이며, 최근 수치는 추후 수정될 수 있다.
  고정 기간 재조회는 `--end 2026-09-10 --days 28`처럼 지정한다.
...
- 홈 첫 페이지에 이미지와 질문으로 구성한 실용 가이드 3개를 배치했다.
  `_data/start_here.yml`에서 진입 주제를 관리한다. 각 글의 실제 URL과 표지 이미지를
  사용하고, 숨김·미발행 글은 노출하지 않는다.
- 테마의 Google Analytics 함수가 DOMContentLoaded 콜백 내부에만 있어
  `window.gtag`를 호출하는 독서 이벤트가 전달되지 않았다. 로컬 include로 분석 큐를
  전역에 먼저 준비해 기존 읽기 시작·완독·공유 이벤트를 연결했다.
- 이전 기간의 읽기 이벤트 부재는 독자가 완독하지 않았다는 뜻이 아니다.
  배포 후 실제 수집을 확인하고, 수집 기간이 쌓인 뒤 비교한다.
