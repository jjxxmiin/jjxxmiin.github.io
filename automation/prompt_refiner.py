...
class PromptRefiner:
    ...
    def refine_prompts(self):
        ...
        for bot_id in bots_to_refine:
            ...
            **작업 지시:**
            1. **대조 분석**: 성과가 좋은 글들과 저조한 글들의 차이점을 분석해. (구조, 어조, 가독성, 그리고 가장 중요한 '사람다움(Human Smell)' 정도)
            2. **문제점 식별**: 저조한 글들이 왜 '기계적'이거나 '지루하게' 느껴졌는지, 체류 시간이 짧은 이유를 분석해.
            3. **프롬프트 개선**: `{bot_id}`의 시스템 프롬프트를 업데이트해.
               - **핵심 요구사항**: 사람 냄새 나게 쓸 것. 완벽한 정리보다 자연스러운 흐름 중시.
               - 개인적인 의견, 고민하는 흔적, 솔직한 장단점 평가가 드러나도록 지시를 강화해.
               - 가독성은 유지하되, 문단 길이의 변화를 주어 리듬감을 만들 것.
