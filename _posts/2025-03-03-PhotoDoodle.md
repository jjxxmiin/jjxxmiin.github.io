...
description: "PhotoDoodle이 소량의 전후 이미지 쌍에서 장식 스타일을 학습하고 배경을 보존하는 구조, 데이터 제작, 마스크, 실행 코드의 검증 기준을 설명합니다."
faq:
  - question: "정말 30~50쌍이면 충분한가요?"
    answer: "논문 조건의 출발점일 뿐 보장 수치가 아닙니다. 쌍 정렬, 스타일 일관성, 대상 다양성과 원하는 품질에 따라 필요한 데이터가 달라집니다."
  - question: "배경 보존은 어떻게 평가하나요?"
    answer: "편집 mask 밖의 pixel, 구조 차이와 mask 안의 스타일 재현을 별도로 측정해야 합니다. 전체 유사도 하나만 보면 실패를 가릴 수 있습니다."
  - question: "학습 쌍에서 무엇을 고정해야 하나요?"
    answer: "원본과 편집본의 crop, 해상도, 조명과 배경을 맞추고 원하는 장식 변화만 남겨야 불필요한 차이를 스타일로 학습하지 않습니다."
image:
...
PhotoDoodle은 사진 전체를 새 스타일로 다시 그리는 모델보다, 원본 배경을 남기면서 몬스터, 빛, 윤곽선 같은 장식 요소를 특정 스타일로 추가하려는 작업에 맞습니다. 새 스타일은 30~50쌍의 편집 전후 예제로 학습하지만, 그 전에 350만 쌍으로 학습한 OmniEditor가 있다는 조건을 빼면 “소량 학습”의 의미가 달라집니다.
...
30~50쌍이라는 숫자보다 중요한 것은 전후 이미지가 같은 장면을 유지하면서 원하는 장식 변화만 보여 주는지입니다. 짝 정렬이 어긋나거나 배경까지 달라지면 모델은 스타일과 불필요한 변화를 함께 학습할 수 있습니다.
...
원본과 편집본의 해상도, crop, 배경을 맞추고 doodle이나 장식만 추가합니다. 카메라 위치, 조명, 압축 품질이 함께 달라지면 적은 데이터에서는 그 차이도 스타일 특징으로 배울 수 있습니다. 각 쌍을 겹쳐 보며 장식 밖 pixel 차이가 큰 표본을 제거하는 검사가 필요합니다.

스타일의 색, 선 굵기, 배치가 너무 비슷한 예만 모으면 새 대상에 일반화하기 어렵고, 반대로 30장 안에 서로 다른 스타일을 섞으면 하나의 일관된 표현을 배우기 어렵습니다. 먼저 한 스타일의 변형 범위를 정의하고 대상과 배경만 다양하게 구성하는 편이 좋습니다.

## 배경 보존과 스타일 재현을 따로 잰다

편집 mask 안에서는 원하는 선과 장식이 나타나는지, mask 밖에서는 원본과 얼마나 같은지를 별도 지표로 봅니다. 전체 image similarity 하나를 쓰면 배경이 넓을 때 스타일을 거의 적용하지 않은 결과가 높은 점수를 받을 수 있습니다. 반대로 장식이 강하지만 얼굴, 제품 모양을 바꾼 결과도 실패입니다.

시험 세트에는 학습과 다른 대상, 복잡한 배경, 가는 구조, mask 경계를 포함합니다. 같은 입력을 여러 seed로 생성해 스타일 일관성과 배경 변화 분산을 기록합니다. 결과가 좋은 샘플만 고르지 말고 실패율과 재생성 횟수를 함께 계산해야 실제 제작 비용을 알 수 있습니다.

## 공개 Code를 재현하는 순서

저장소의 환경과 checkpoint 요구를 먼저 확인하고 작은 sample로 inference를 실행합니다. 이후 자신의 data loader가 원본, 편집본, mask를 올바른 순서와 범위로 읽는지 한 batch를 시각화합니다. 학습 전후에 같은 validation set과 seed를 유지해야 개선을 비교할 수 있습니다.

기존 실행 조각에 placeholder path나 생략된 preprocessing이 있다면 그대로 완전한 명령으로 간주하지 않습니다. 해상도를 늘릴 때 memory와 결과가 어떻게 변하는지, fine-tuning 중 배경 보존이 언제 악화되는지를 checkpoint별로 확인합니다. PhotoDoodle이 맞는지는 적은 쌍의 숫자보다 자신의 스타일에서 원하는 변화와 보존 영역을 안정적으로 분리하는지로 결정됩니다.
...
논문 조건의 출발점일 뿐 보장 수치가 아닙니다. 쌍 정렬, 스타일 일관성, 대상 다양성과 원하는 품질에 따라 필요한 데이터가 달라집니다.

### 배경 보존은 어떻게 평가하나요?

편집 mask 밖의 pixel, 구조 차이와 mask 안의 스타일 재현을 별도로 측정해야 합니다. 전체 유사도 하나만 보면 실패를 가릴 수 있습니다.

### 학습 쌍에서 무엇을 고정해야 하나요?

원본과 편집본의 crop, 해상도, 조명과 배경을 맞추고 원하는 장식 변화만 남겨야 불필요한 차이를 스타일로 학습하지 않습니다.
